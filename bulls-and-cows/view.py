...
def showGameScreen():
    ...
    game_rule = ('''
                 Guess the Unique 3-DIGIT number!
                 S(Strike) : If location and number are the same
                 B(Ball): The location is different, but if that number is included
                 O(Out) : If submitted number is not included
                 ''')
